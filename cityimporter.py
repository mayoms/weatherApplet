...
class ReadCityData():
    ...
    def parse_country_state(self, city_geodata):
        ...
        if city_geodata[8] == 'us':
            return city_geodata[2], city_geodata[8], city_geodata[10], city_geodata[4], city_geodata[5]
        # If not in the united states, first get region name from admin1CodesASCII.txt, using region ID number
        # included in original file outside of the US, CA, GB, etc..
        try:
            region = self.region_codes[city_geodata[8] + '.' + city_geodata[10]]
        except KeyError:
            region = None
        return city_geodata[2], city_geodata[8], region, city_geodata[4], city_geodata[5]
